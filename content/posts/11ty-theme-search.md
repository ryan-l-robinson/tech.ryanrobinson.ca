...
Some of this process is handled in the 11ty-theme's config.js file, including passing through the active JavaScript that the end user will need:
...
And creating the filter that we'll need to generate the search index:
...
Essentially all this is doing is passing through the relevant data from every page where it can be quickly scanned by elasticlunr.

A file that uses that filter is done by creating a search-index.json.njk file on each site:
...
---
{{ collections.posts | search | dump | safe }}
{% endraw %}
```

Finally, when the end user visits the site, the JavaScript they will activate are the default one from elasticlunr plus this one in the 11ty-theme that actually performs the search, including boosts depending on where the data is housed and a slight boost for newer content over older content.
